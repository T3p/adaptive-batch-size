...
class LQGND(gym.Env):
    ...
    def step(self, action, render=False):
        ...
        if self.sigma_noise > 0:
            noise = self.np_random.randn(dim) * self.sigma_noise
    ...
    def reset(self, state=None):
        if state is None:
            self.state = np.array([self.np_random.uniform(low=-self.max_pos,
                                                          high=self.max_pos) for _ in range(self.dim)])
        else:
    ...
    def _computeP2(self, K):
        ...
        if np.array_equal(self.A, I) and np.array_equal(self.B, I):
            P = np.dot((self.Q + np.dot(K.T, np.dot(self.R, K))), np.linalg.inv(I - self.gamma *
                                                             (I + 2 * K + K **
                                                              2)))
