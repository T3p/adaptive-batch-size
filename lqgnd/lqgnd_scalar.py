...
if __name__ == '__main__':
    ...
    while True: 
        ...
        grad_Js = np.array(grad_Js) 
        k = np.argmax(abs(grad_Js))
        ...
        #Stopping condition due to fixed batch size
        print 'infgrad:', infgrad, 'eps:', actual_eps
        if actual_eps >= infgrad:
            print "Error is leq than gradient!"
            break
        print np.maximum(abs(grad_Js) - actual_eps,np.zeros(m))
        alpha = norm(np.maximum(abs(grad_Js) - actual_eps,np.zeros(m)), 2)**2/(2*c*norm(abs(grad_Js) + actual_eps, 1)**2) 
        if verbose>0:
            print 'alpha:', alpha
