...
if __name__ == '__main__': 
    ...
    with tf.Session() as sess:
        ...
        while True: 
            ...
            M_phi = np.amax(traces[:,:,0:s_dim])
            R = np.amax(traces[:,:,s_dim+a_dim])
           ...
           #Performance estimation
            ...
            grad_samples = Parallel(n_jobs=n_cores)(delayed(grad_estimator)(scores[:,:,j],disc_rewards) 
                        for j in xrange(m))
            grad_Js = np.mean(grad_samples,0)
            sample_var = max(np.var(grad_samples,0))
            print 'Grads:', max(grad_Js)
            infgrad = max(abs(grad_Js))
            ...
            alpha_k = (infgrad - actual_eps)**2/(2*c*(infgrad + actual_eps)**2)
            if verbose>0:
                print 'alpha', alpha
            N = N_star
            ...
            alpha = np.zeros(m)
            alpha[k] = alpha_k
            pol.update_outer_layer(alpha_k*np.array(grad_Js))
