...
class NormalPolicy(object):
    ...
    def _build(self):
        ...
        with tf.variable_scope(self.tf_scope):
            self.mu,self.outer_layer = create_mlp(self._state, layer_size=self._mean_layers + [self._action_dim], activ=self.mean_activ)
            ...
            if sum(self._mean_layers)>0:
                self.n_outer_weights = self._mean_layers[-1]+1
            else:
                self.n_outer_weights = len(self.weights)
            
            nb_weights = 0
            for w in weights: 
                dim = np.prod(w.shape.as_list())
                nb_weights+=dim
            self.n_weights = nb_weights
            ...
            self.flat_outer_scores = self.flat_scores[-self.n_outer_weights:]
    ...
    def outer_log_gradients(self,state,action,sess=None):
        sess = sess or tf.get_default_session()
        gradients = sess.run(self.flat_outer_scores,{self._state: state, self._action: action})
        return gradients
    ...
    def get_outer_weights(self,sess=None):
        sess = sess or tf.get_default_session()
        w = self.get_weights(sess)
        return w[-self.n_outer_weights:]

    def update(self,delta_w,sess=None):
        sess = sess or tf.get_default_session()
        sess.run(self.update_weights,{self.delta_weights: delta_w})

    def update_outer_layer(self,delta_w,sess=None):
        sess = sess or tf.get_default_session()
        full_delta_w = np.zeros(self.n_weights) 
        full_delta_w[-self.n_outer_weights:] = delta_w
        self.update(full_delta_w,sess)  
 
    def reset(self,custom_w,sess=None):
        sess = sess or tf.get_default_session()
        sess.run(self.reset_weights,{self.custom_weights: custom_w})       

    def get_features(self,state,sess=None):
        sess = sess or tf.get_default_session()
        return sess.run(self.outer_layer,{self._state: state})

    def save_weights(self,filename,sess=None):
        weights = self.get_weights(sess)
        np.save(filename,weights)

    def load_weights(self,filename,sess=None):
        saved_w = np.load(filename)
...
if __name__ == "__main__":
    ...
    pol = NormalPolicy(action_size, [5],
                       [],-1,1,min_std=0.1,
    ...
    with tf.Session() as sess:
        ...
        #pol.load_weights()
        #print pol.get_weights(), '\n'
        #print sess.run(pol.n_outer_weights,{pol._state : s})
        #print pol.n_outer_weights
        #print pol.get_outer_weights(), '\n'
        #pol.update_outer_layer(np.array([1,2,3,4,5,6]))
        #print pol.get_weights(), '\n'
        #print pol.get_outer_weights(), '\n'
        print pol.get_features(s)
