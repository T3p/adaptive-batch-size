...
def cheb_reinforce(R,M_phi,H,delta,sigma,gamma,grad_range=None,sample_var=None,grad_J=None,c=None):
    d =  math.sqrt((R**2*M_phi**2*H*(1-gamma**H)**2)/ \
                (sigma**2*(1-gamma)**2*delta)) 
    return d,0,0

def cheb_gpomdp(R,M_phi,H,delta,sigma,gamma,grad_range=None,sample_var=None,grad_J=None,c=None):
...
def hoeffding(R,M_phi,H,delta,sigma,gamma,grad_range,sample_var=None,grad_J=None,c=None):
    assert delta<1
    d = grad_range*math.sqrt(math.log(2/delta)/2)
    return d,0,0

def bernstein(R,M_phi,H,delta,sigma,gamma,grad_range,sample_var,grad_J,c):
    assert delta<1
    d = math.sqrt(2*math.log(3.0/delta)*sample_var)
    f0 = 3*grad_range*math.log(3.0/delta)
    N0 = min(N_max,max(N_min,int(((d + math.sqrt(d**2 + 4*f0*abs(grad_J))) \
            /(2*abs(grad_J)))**2) + 1))
    print 'N0:', N0
    ups_max = -np.inf
    N_opt = N0
    for n in range(N0,N_max+1):
        eps = d/math.sqrt(n) + f0/n
        upsilon = (abs(grad_J) - eps)**4/ \
                    (4*c*(abs(grad_J) + eps)**2*n)
        if upsilon>ups_max:
            ups_max = upsilon
            N_opt = n
    return d,f0/N_opt,f0,N_opt
        

def iter_bernstein(R,M_phi,H,delta,sigma,gamma,grad_range,sample_var,grad_J,c=None):
...
if __name__ == '__main__':
    ...
    while True: 
        ...
        d,f,f0,N_star = stat_bound(R,M_phi,H,delta,sigma,gamma,rng,sample_var,grad_J,c)
