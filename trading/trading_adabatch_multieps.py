...
def sample_bernstein(R,M_phi,sigma,grad_abs,sample_var,c,sample_rng):
    rng = sample_rng

    ups_max = -np.inf
    k = 0
    for j in range(m):
        d = math.sqrt(2*math.log(3.0/delta)*sample_var[j])
        f = 3*rng[j]*math.log(3.0/delta)
        eps = d + f
        upsilon = (grad_abs[j] - eps)**4/ \
                (4*c*(grad_abs[j] + eps)**2)
        if upsilon>ups_max:
            ups_max = upsilon
            k = j

    infgrad = grad_abs[k]
    d = math.sqrt(2*math.log(3.0/delta)*sample_var[k])
    f = 3*rng[k]*math.log(3.0/delta)
    N_0 = min(N_max,max(N_min,int(((d + math.sqrt(d**2 + 4*f*infgrad)) \
            /(2*infgrad))**2) + 1))
    ups_max = -np.inf
    eps_star = np.inf
    N_star = N_0
    for N in range(N_0+1,N_max+1):
        d = math.sqrt(2*math.log(3.0/delta)*sample_var[k])
        f = 3*rng[k]*math.log(3.0/delta)
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        while True: 
            ...
            navs,max_states = zip(*[trajectory(n,traces) for n in range(N)])
            scores = traces[:,:,0:m]
            rewards = traces[:,:,m]
            ...
            for j in range(m):
                ...
                sample_var[j] = np.var(grad_samples[j,:],ddof=1)
            grad_abs = abs(grad_samples)
            rng = [max(grad_abs[j,:]) - min(grad_abs[j,:]) for j in range(m)]
            print 'max range:', max(rng)
            grad_abs = abs(grads_J)
            d,f,eps_star,N_star,k = stat_bound(R,M_phi,sigma,grad_abs,sample_var,c,rng)
            infgrad = grad_abs[k]
            del grad_samples
            ...
            if N_tot>N_maxtot:
                print "Max N reached"
                break
            if False:#iteration>=max_iter:
                print "Max iteration reached"
                break

        pol.save_weights(idx=21)
