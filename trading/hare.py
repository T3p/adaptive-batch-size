...
def sample_bernstein(R,M_phi,sigma,grads,sample_vars,c,sample_ranges):
    ...
    for j in range(m):
        ...
        ups = (grads[j] - eps)**4/(4*c[j]*(grads[j] + eps)**2)
        if ups > ups_max:
            ups_max = ups
            k = j
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        m = len(theta)
        pol.reset(np.array([0]*obs_size+[0]))
        ...
        while True: 
            ...
            alpha = 1e-1/math.sqrt(iteration)#(infgrad - actual_eps)**2/(2*c[k]*(infgrad + actual_eps)**2) 
            if verbose>0:
                    print 'alpha:', alpha
            ...
            if verbose>0:
                ...
                if eps_star>=infgrad:
                    print 'eps too high!'
            #N = N_fix#
            N = 100#min(N_max,max(N_min,N_star)) 
            print 'Next N:', N
